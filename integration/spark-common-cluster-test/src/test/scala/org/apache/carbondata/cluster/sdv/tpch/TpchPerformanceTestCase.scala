 ...
 */
class TpchPerformanceTestCase extends QueryTest with BeforeAndAfterAll  {
...
//    ("Q21", "select s_name, count(*) as numwait from supplier, lineitem l1, orders, nation where s_suppkey = l1.l_suppkey and o_orderkey = l1.l_orderkey and o_orderstatus = 'F' and l1.l_receiptdate > l1.l_commitdate and exists ( select * from lineitem l2 where l2.l_orderkey = l1.l_orderkey and l2.l_suppkey <> l1.l_suppkey ) and not exists ( select * from lineitem l3 where l3.l_orderkey = l1.l_orderkey and l3.l_suppkey <> l1.l_suppkey and l3.l_receiptdate > l3.l_commitdate ) and s_nationkey = n_nationkey and n_name = 'SAUDI ARABIA' group by s_name order by numwait desc, s_name"),
  ...
  val loadParquetQueries = Seq(
    ...
    ("ORDERS",s"""insert into table ORDERS partition(O_ORDERDATE) select
      O_ORDERKEY,
      O_CUSTKEY,
      O_ORDERSTATUS,
      O_TOTALPRICE,
      ...
      O_SHIPPRIORITY,
      O_COMMENT,
      o_orderdate from $srcDatabase.ORDERS""".stripMargin),

    ("LINEITEM", s"""insert into table lineitem partition(L_SHIPDATE) select L_ORDERKEY,L_PARTKEY,L_SUPPKEY,L_LINENUMBER,L_QUANTITY,L_EXTENDEDPRICE,L_DISCOUNT,L_TAX,L_RETURNFLAG,L_LINESTATUS,L_COMMITDATE,L_RECEIPTDATE,L_SHIPINSTRUCT,L_SHIPMODE,L_COMMENT, l_shipdate from $srcDatabase.lineitem DISTRIBUTE BY l_shipdate""".stripMargin),

    ("PARTSUPP",s"""insert into table PARTSUPP select * from $srcDatabase.partsupp""".stripMargin),
    ...
    ("PART",s"""insert into TABLE PART partition(P_BRAND)
    select
    P_PARTKEY,
    P_NAME,
    P_MFGR,
    ...
    P_COMMENT,
    P_BRAND
    from $srcDatabase.part""".stripMargin),

    ("CUSTOMER",s"""insert into table CUSTOMER partition(C_MKTSEGMENT) select
    C_CUSTKEY,
    C_NAME,
    C_ADDRESS,
    C_NATIONKEY,
    ...
    C_COMMENT,
    C_MKTSEGMENT
    from $srcDatabase.customer""".stripMargin)
  )
  ...
  def createAndLoadTablesParquet(): Unit = {
    ...
    loadParquetQueries.foreach {q =>
      ...
      println("Time is "+ t +" and Count of "+q._1 +" is "+ (sql("select count(*) from "+ q._1 ).collect()))
