...
//    spark.sql("show create table source").show(false)
//    spark.sessionState.catalog.asInstanceOf[CarbonSessionCatalog].getClient().runSqlHive("show create table source").foreach(println)
    spark.sql("DROP TABLE IF EXISTS source_native")
    spark.sessionState.catalog.asInstanceOf[CarbonSessionCatalog].getClient().runSqlHive(
      s"""
         |CREATE EXTERNAL TABLE `source_native`(
         |  `shortfield` smallint COMMENT '',
         |  `intfield` int COMMENT '',
         |  `bigintfield` bigint COMMENT '',
         |  `doublefield` double COMMENT '',
         |  `stringfield` string COMMENT '',
         |  `timestampfield` timestamp COMMENT '',
         |  `decimalfield` decimal(18,2) COMMENT '',
         |  `datefield` date COMMENT '',
         |  `charfield` string COMMENT '')
         |partitioned by (`floatfield` double COMMENT '')
         |ROW FORMAT SERDE
...
//    // scalastyle:off
    ...
    spark.sql("insert overwrite table source_native partition(floatField) select * from source")
//
    spark.sql("select * from source_native").show()
//    // scalastyle:on
//
//    spark.sql(
//      s"""
