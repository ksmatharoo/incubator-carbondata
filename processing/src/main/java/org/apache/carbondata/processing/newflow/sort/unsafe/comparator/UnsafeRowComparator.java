...
public class UnsafeRowComparator implements Comparator<UnsafeCarbonRowHolder> {
   ...
   */
  public UnsafeRowComparator(boolean[] noDictionaryColMaping, UnsafeCarbonRowPage rowPage) {
   ...
   */
  public int compare(UnsafeCarbonRowHolder rowL, UnsafeCarbonRowHolder rowR) {
    int diff = 0;
    ...
    for (boolean isNoDictionary : noDictionaryColMaping) {
      if (isNoDictionary) {
        ...
        CarbonUnsafe.unsafe.copyMemory(baseObject, rowA + sizeA, byteArr1,
            CarbonUnsafe.BYTE_ARRAY_OFFSET, sizeA);
        sizeA += aShort1;

        short aShort2 = CarbonUnsafe.unsafe.getShort(baseObject, rowB + sizeB);
        byte[] byteArr2 = new byte[aShort2];
        sizeB += 2;
        CarbonUnsafe.unsafe.copyMemory(baseObject, rowA + sizeB, byteArr2,
            CarbonUnsafe.BYTE_ARRAY_OFFSET, sizeB);
        sizeB += aShort2;
