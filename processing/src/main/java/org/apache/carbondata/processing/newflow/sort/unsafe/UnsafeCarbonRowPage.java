...
public class UnsafeCarbonRowPage {
  ...
  public UnsafeCarbonRowPage(boolean[] noDictionaryDimensionMapping, int dimensionSize,
    ...
    buffer = new IntPointerBuffer(sizeInMB, unsafe);
  ...
  public Object[] getRow(long address, Object[] rowToFill) {
    ...
    int nullSetSize = nullSetWords.length * 8;
    Arrays.fill(nullSetWords, 0);
    CarbonUnsafe.unsafe.copyMemory(dataBlock.getBaseObject(), address + size, nullSetWords,
        CarbonUnsafe.LONG_ARRAY_OFFSET, nullSetSize);
    size += nullSetSize;
  ...
  public void freeMemory() {
    buffer.freeMemory();
    if (keepInMemory) {
      UnsafeMemoryManager.INSTANCE.freeMemory(buffer.getTotalSize());
    }
  }
